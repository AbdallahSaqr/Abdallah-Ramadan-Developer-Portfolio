...
Personal portfolio: a landing page plus privacy and terms pages, served in
English and Arabic (RTL), with light and dark themes and no backend.
...
src/
  app/
    [locale]/
      ...
      privacy/, terms/    Legal pages, server-rendered, no client JS
  ...
  lib/
    ...
    motion.ts             The one fade variant used site-wide
    use-*.ts              Small client hooks
tests/
  unit/                   Vitest + Testing Library (components, hooks, config)
  e2e/                    Playwright (routing, legal pages, SEO, headers, a11y)
```
...
- **Client wordmarks:** `src/assets/logos/`, attached to a project via its
  `logo` field.
- **Legal copy:** the `privacy.*` and `terms.*` keys in `src/lib/i18n.ts`.
...
The visual language is deliberately plain: warm neutrals, one ochre accent, flat
colour, square-shouldered controls, and a single short fade as blocks enter the
viewport. No gradients, pill shapes, emoji icons, em dashes, invented metrics or
looping animation. `tests/unit/design-rules.test.ts` and the house-rules block in
`tests/e2e/page.spec.ts` enforce this, so a regression fails the suite rather
than shipping. `AGENTS.md` carries the full list.

## Launch checklist

1. Connect the custom domain and set `NEXT_PUBLIC_SITE_URL` to it
2. Favicon: done, generated by `src/app/icon.tsx` and `src/app/apple-icon.tsx`
3. No "made with AI" badge: done, the site has never carried one
4. Privacy policy: done, `/[locale]/privacy`
5. Terms and conditions: done, `/[locale]/terms`

Only item 1 is outstanding, and it is done in the Vercel dashboard.
